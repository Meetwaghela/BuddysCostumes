...
import Ins9 from "../../components/assets/Ins9.mp4"; // Corrected from .mp4 if it was .jpeg or vice versa in original
...
import Ins13 from "../../components/assets/Ins13.mp4"; // Corrected from .mp4 if it was .jpeg or vice versa in original
import Ins14 from "../../components/assets/Ins14.jpeg";

const Stonework = () => {
  const [activeIndex, setActiveIndex] = useState(0);
  const [clickedIndex, setClickedIndex] = useState(null); 
  // const [theme, setTheme] = useState(null); // 'theme' state isn't used, can be removed if not needed later
  ...
  const images = [
    ...
    // **IMPORTANT:** Check the actual file type for Ins9 in your assets folder. If it's a JPEG, change 'video' to 'image'.
    ...
    // **IMPORTANT:** Check the actual file type for Ins13 in your assets folder. If it's a JPEG, change 'video' to 'image'.
  ...
  return (
    <div>
      <div className="text-3xl font-bold text-center py-20 mb-20 dark:text-white">
        <h1 className="mb-4">Embroidery with Stones</h1> {/* Added mb-4 for margin-bottom */}
      ...
      <div className="image-container">
        {images.map((item, index) => ( // Changed 'image' to 'item' for clarity when handling mixed types
          <div
            key={index}
          ...
          >
            {item.type === "image" ? (
              ...
              <video
                src={item.src}
                alt={item.alt} // 'alt' isn't directly used by video, but good for accessibility/fallback text
                className="hover-image" // You can apply similar styling
                loop // Loop the video
                muted // Mute by default, crucial for autoplay in most browsers
                autoPlay // Attempt to autoplay
                playsInline // Important for iOS to play video inline rather than fullscreen
                preload="auto" // Helps load video data
              >
